...
/**
 * @title DeployApxUSDPool
 * @notice Deployment script for deploying an apxUSD-USDC Curve stableswap pool on mainnet
 * @dev Reads configuration from config.toml and deploy/mainnet.toml
 *
 * Usage:
 *   NETWORK=mainnet forge script cmds/curve/DeployApxUSDPool.sol:DeployApxUSDPool --rpc-url <RPC_URL> --broadcast --verify --interactives 1 -vvvv
 */
contract DeployApxUSDPool is BaseDeploy {
    // ========================================
    // State
    // ========================================
    ...
    function run() public {
        ...
        usdc = config.get(chainId, "usdc_address").toAddress();
        vm.label(usdc, "USDC");
        ...
        deployConfig.set(chainId, "apxUSDRateOracle_block", block.number);
    ...
    function _deployPool() internal returns (address) {
        ...
        console2.log("\n=== Deploying Pool ===");
        console2.log("Coins[0] (apxUSD):", coins[0]);
        console2.log("Coins[1] (USDC):  ", coins[1]);
